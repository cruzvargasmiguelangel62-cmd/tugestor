...
const EditorView: React.FC<EditorViewProps> = ({ view, setView, quotes, activeQuote, setActiveQuote, saveQuote, catalog, clientSuggestions = [] }) => {
  ...
  return (
    <Screen className="bg-white py-0 sm:py-4 md:py-6 lg:py-8">
      ...
      {/* Header - Responsive */}
      <div className="sticky top-[60px] md:relative md:top-0 bg-white z-30 px-4 sm:px-6 md:px-0 py-3 md:py-0 mb-4 md:mb-8 flex justify-between items-center md:block border-b md:border-b-0 border-slate-100">
        <div className="flex items-center justify-between w-full md:w-auto md:mb-6">
          <button onClick={() => setView('home')} className="p-2 -ml-2 md:hidden hover:bg-slate-100 rounded-lg transition-colors">
            <ChevronLeft size={24} />
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10 px-4 md:px-0 pt-2 md:pt-0">
