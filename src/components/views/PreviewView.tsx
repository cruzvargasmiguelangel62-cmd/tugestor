...
const PreviewView: React.FC<PreviewViewProps> = ({ setView, activeQuote, profile, onToggleStatus }) => {
  ...
  return (
    <Screen className="bg-slate-100 lg:bg-slate-50 py-0 sm:py-4 md:py-6 lg:py-8">
      ...
      <div className="bg-white px-4 sm:px-6 py-4 md:py-5 shadow-sm md:rounded-2xl mb-6 md:mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sticky top-0 md:relative z-20 no-print border border-slate-200">
        <button
          onClick={() => setView('history')}
          className="flex items-center gap-3 text-sm font-extrabold text-slate-600 hover:text-slate-900 transition-all md:hidden group"
        >
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 items-center w-full sm:w-auto">
          <button
            ...
            className={`flex shrink-0 items-center gap-2 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-bold transition-all ${isPaid ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-orange-100 text-orange-700 hover:bg-orange-200'
              }`}
          >
